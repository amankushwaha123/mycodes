  ...
  <form action="" method="POST">
    ...
    <div class="form-row">
      <div class="form-group col-md-6">
       ...
       <?php   $qlm="select empName from technician_tb";
               $tres=$conn->query($qlm);
              
        ?>
        <select name='assigntech' class='form-control'>
        <option value="">select any one</option>
        <?php for($i=1;$i<=$tres->num_rows;$i++){
                  $row2=$tres->fetch_assoc(); 
        ?>
        <option value="<?php echo $row2['empName'] ?>"><?php echo $row2['empName'] ?> </option>
        <?php } ?>>
